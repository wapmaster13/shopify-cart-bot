...
export async function action({ request }: { request: Request }) {
    ...
    // Save to DB
    const newRule = await prisma.giftRule.create({
        data: {
            shop,
            triggerAmount,
            giftVariantId,
    ...
    try {
        ...
        const metafieldValue = JSON.stringify({
            triggerAmount: newRule.triggerAmount,
            giftVariantId: newRule.giftVariantId,
            isActive: true
        });
...
export default function NewRule() {
    ...
    const handleBrowse = async () => {
        ...
        if (selected && selected.selection && selected.selection.length > 0) {
            ...
            // handle fallback if it returns array directly
